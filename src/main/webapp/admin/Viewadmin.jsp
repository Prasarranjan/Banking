...
<div class="wrapper">
    ...
    <main class="page-content">
        ...
        <div class="modal fade" id="addAdminModal2" tabindex="-1" aria-labelledby="addBankModalLabel"
             aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addBookModalLabel2">Update Admin</h5>
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    for (var key in data) {
                        if (data.hasOwnProperty(key)) {
                            ...
                            s += "<a  class='text-info bank_edit' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Edit' data-id='" + data[key].adminId + "' id='" + data[key].adminId + "'><i class='bi bi-pencil-fill'></i></a>";
                            s += "<a  class='text-danger bank_delete' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Delete' data-id='" + data[key].adminId + "' id='" + data[key].adminId + "'><i class='bi bi-trash-fill'></i></a>";
                            s += "</div></td></tr>";
                        }
                        i++;
                    }
...
<script>
    $(document).on('click', '.bank_edit', function () {
        let id = $(this).attr('id');
        $('#addAdminModal2').modal('show');
        $.ajax({
            type: "Post",
            url: '../BankServlet',
            data: {"id": id, "event": "editAdmin"},
            dataType: "JSON",
            success: function (response) {
                $("#Id").val(response[0].adminId);
                $("#Name").val(response[0].adminName);
                $("#Email").val(response[0].adminEmail);
                $("#Phone").val(response[0].adminPNum);
                $("#Password").val(response[0].adminPassword);
...
<script>
    ...
    const tableBody = document.getElementById('viewAdmin');
